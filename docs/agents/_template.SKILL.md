...
> Type: Vertical Module Agent | Module: src/core-modules/{{MODULE_PATH}}/ or src/app-modules/{{MODULE_PATH}}/
...
2. Read the module's MODULE.md for module spec (path is src/core-modules/{{MODULE_PATH}}/ or src/app-modules/{{MODULE_PATH}}/ per REGISTRY).
...
7. Publish {{MODULE_NAME}}* events via EventBridge for state changes.
